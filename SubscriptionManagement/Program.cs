...
using Serilog;
using SubscriptionManagement.Consumer;
...
IHost host = Host.CreateDefaultBuilder(args)
    ...
    {
        services.AddTransient<SubscritionConsumer>();
        services.AddMassTransit(bus =>
        {
            bus.UsingRabbitMq((context, cfg) =>
            {
                ...
                {
                    cfg.ConfigureConsumer<SubscritionConsumer>(context);
                });

            });

            bus.AddConsumer<SubscritionConsumer>();
        });
    })
